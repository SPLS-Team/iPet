...
export function icon(name, { size = 16, label = "" } = {}) {
  ...
  if (!path) return "";
  const a11y = label
    ? ` role="img" aria-label="${escapeAttr(label)}"<title>${escapeHtml(label)}</title>`
    : ' aria-hidden="true"';
  return `<svg class="icon" width="${size}" height="${size}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"${a11y}>${path}</svg>`;
}
