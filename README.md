...
- **Transparent, frameless, always-on-top window** — drag anywhere, mouse-passthrough toggle, and a "compact" mode that shrinks to a small translucent **glass pill** floating on the desktop (showing live status or the pomodoro countdown; click or right-click to expand back).
- **Pomodoro timer** — standard 25/5 focus–break cycle with a long break every 4 sessions, start/pause/skip/reset, OS notifications on phase transitions, adjustable durations + auto-start toggles (persisted). Completed sessions are logged to SQLite and shown as a today/7d/30d completed-count + per-day trend in the Usage view. Visible in the talk workspace and on the glass pill.
...
- **Function-calling with three tool kinds under one spec** — **20 built-in tools** (see below), user-defined HTTP tools, and `local` subprocess tools, all described by a `tool.json` manifest (schemaVersion 1 = http, 2 = +local) and runtime-hot-pluggable (per-request DB read, no cache).
...
| `set_compact_window`     | Shrink to the translucent glass-pill "compact" mode          |
...
- `web_search` — keyless web search via DuckDuckGo: prioritizes Instant Answer abstracts and merges real HTML results (title + decoded URL + snippet). Timeout + body-size caps, no API key.
...
**App usage (desktop "screen time")**
- `app_usage_stats` — aggregate foreground-app usage time over `today` / `7d` / `30d`. Backed by a background sampler that polls the foreground window every 15s and credits the interval to the owning app, skipping idle time past the configured threshold. Windows (`GetForegroundWindow`), macOS (`NSWorkspace` + CoreGraphics idle) and Linux/X11 (`xprop`) are supported; pure-Wayland sessions degrade to unsupported. Rows persist in SQLite (`app_usage`, schema v4) and stay on-device.
...
- **Compact mode** (`◱` button) shrinks to a translucent glass-pill that you can drag around with `mousedown`; click (without dragging) expands back to the talk workspace.
...
- **透明、无边框窗口** —— 任意位置拖拽、鼠标穿透开关、"紧凑"模式缩小为一个半透明**玻璃药丸**小窗浮于桌面（显示实时状态或番茄钟倒计时，单击或右键展开回对话）；标题栏置顶按钮一键切换置顶状态（默认非置顶）。
- **番茄钟** —— 标准 25 分钟专注 / 5 分钟休息循环，每 4 个番茄一次长休息；支持开始/暂停/跳过/重置、时段切换系统通知、可调时长与自动开始开关（持久化）；完成的番茄会记入 SQLite，在「用量」分区按 today/7d/30d 展示完成数与按日趋势；在对话区与玻璃药丸上都能看到倒计时。
...
- **流式对话**，对接任何 OpenAI 兼容的 `/chat/completions` 接口；实时打字 + 思考计时器 + 可展开「思考链」（自动捕获 `reasoning_content`）；用 `marked` + `DOMPurify` 渲染 Markdown（GFM 表格、任务列表、代码块）。
- **多服务商预设 + 模型列表自动获取** —— 模型页内置 OpenAI / DeepSeek / Anthropic 兼容代理 / OpenRouter / 硅基流动 / Moonshot / 本地 Ollama 等 Base URL 预设，一键填入；「刷新模型列表」从 `/models` 拉取可用模型并填充输入框候选。
- **三类工具、统一规范的 function calling** —— **20 个内置工具**（见下）、自定义 HTTP 工具、以及 `local` 子进程工具，全部由 `tool.json` 描述（schemaVersion 1=http / 2=+local），运行时热插拔（每请求实时查 DB，无缓存）。内置工具实现为 `ipet-tool-desktop-tools` crate 的 Rust 函数，经 dispatcher 路由，并在 Tokio 阻塞线程池上调度，慢工具不会卡住运行时。
- **20 个内置桌面工具** —— `get_system_status`、`scan_disk`、`search_files`、`read_text_file`、`git_status`、`list_processes`、`network_status`、`clipboard_read`、`open_path`、`recent_system_errors`、`package_scripts`、`run_project_check`、`disk_cleanup_candidates`、`create_note`、`weather_lookup`、`web_search`、`screenshot_ocr`，用于长期记忆的 `memory_save` / `memory_search`，以及统计前台使用时长的 `app_usage_stats`。每个网络/子进程调用都带超时和输出上限。
- **应用使用时长（桌面版"屏幕使用时间"）** —— 后台每 15 秒采样当前前台窗口，把这段时间记到对应应用名下，跳过超过空闲阈值的挂机时间，按天聚合存入本地 SQLite（`app_usage`，schema v4）。可在「控制中心 → 用量」查看，模型也能通过 `app_usage_stats` 工具查询；默认开启，可在「系统」分区关闭。前台采样支持 Windows（`GetForegroundWindow`）、macOS（`NSWorkspace` + CoreGraphics 空闲检测）、Linux/X11（`xprop`）；纯 Wayland 会话降级为不支持。
...
- **系统通知** —— 经 `tauri-plugin-notification` 走系统通知中心：可选「回答完成时通知」与「自动系统检查发现高负载（CPU/内存 ≥ 85%）时告警」，在「控制中心 → 系统」里开关。
...
iPet 内置 20 个工具，实现为 `ipet-tool-desktop-tools` crate 的 Rust 函数，经 `ToolDispatcher::dispatch_builtin` 路由，并在 Tokio 阻塞线程池上调度（`spawn_blocking`），慢工具不会卡住运行时。每个网络/子进程调用都带超时和输出上限；只读工具在其 `tool.json` 标注 `readOnly: true`，其余带明确安全说明。
...
- `web_search` —— 免 Key 网页搜索，经 DuckDuckGo：优先返回 Instant Answer 摘要，并合并 HTML 搜索页的真实网页结果（标题 + 解码后的 URL + 摘要）。带超时与响应体大小上限，无需 API Key。
