...
<p>
  ...
  <img alt="Platform" src="https://img.shields.io/badge/platform-Windows_|_macOS-0078D6?logo=windows&logoColor=white">
...
The project is built with **Tauri 2** (Rust backend + WebView frontend), uses **Vite** for the frontend build, and ships as a Windows executable (NSIS installer + portable zip) and a macOS DMG.
...
**Windows**
```powershell
# Prerequisites: Node.js 18+, Rust stable toolchain
npm install
npm run tauri:dev
```
...
**macOS**
```zsh
# Prerequisites: Node.js 18+, Rust stable toolchain, Xcode Command Line Tools
npm install
npm run tauri:dev
```

> **macOS Gatekeeper**: the DMG is not notarized. On first launch, **right-click the app → Open**, then click **Open** in the dialog. Alternatively, remove the quarantine flag:
> ```zsh
> xattr -d com.apple.quarantine /Applications/iPet.app
> ```
> Either way, macOS remembers the choice — you only need to do this once.
...
IPET_LOG=ipet_lib=debug ./ipet
IPET_LOG=ipet_lib::tool_dispatcher=trace ./ipet
```
...
cargo test --release --lib --manifest-path src-tauri/Cargo.toml  # Rust unit tests
```
...
cargo test --lib --manifest-path src-tauri/Cargo.toml   # --release not needed on macOS
```

> **Why `--release` on Windows?** The debug `cargo test` build statically links the full Tauri stack and the resulting >200 MiB exe trips a Windows loader bug (`STATUS_ENTRYPOINT_NOT_FOUND`) before any test code runs. Release-mode optimizations shrink it past the threshold. This is a Windows-specific issue — macOS is unaffected.
...
| Artifact      | Path                                                            |
|---------------|-----------------------------------------------------------------|
| Windows exe   | `src-tauri/target/release/ipet.exe`                             |
| NSIS installer| `src-tauri/target/release/bundle/nsis/iPet_0.3.1_x64-setup.exe` |
| Portable zip  | `src-tauri/target/release/bundle/zip/iPet_0.3.1_x64_portable.zip` |
| macOS DMG     | `src-tauri/target/release/bundle/dmg/iPet_0.3.1_aarch64.dmg`   |

> **macOS users**: the DMG is not notarized. On first launch, **right-click the app → Open**, then click **Open** in the dialog. Alternatively:
> ```zsh
> xattr -d com.apple.quarantine /Applications/iPet.app
> ```
...
- macOS / Linux smoke tests (Windows + macOS builds are green in CI; Linux is portable but unverified).
...
项目使用 **Tauri 2**（Rust 后端 + WebView 前端），前端构建走 **Vite**，最终产物包含 Windows 可执行文件（NSIS 安装包 + portable zip）和 macOS DMG。
...
> **macOS Gatekeeper**：DMG 未做公证。首次启动请**右键点击 app → 打开**，在弹出的对话框中点**打开**；也可以命令行去隔离：
> ```zsh
> xattr -d com.apple.quarantine /Applications/iPet.app
> ```
> 只需一次，后续直接双击即可。
...
npm run build        # 前端构建
npm run tauri:dev    # Tauri 开发模式（Vite + Rust）
cargo check --manifest-path src-tauri/Cargo.toml   # Rust 类型检查
cargo test --release --lib --manifest-path src-tauri/Cargo.toml  # Rust 单元测试
```
...
npm run tauri:dev
cargo check --manifest-path src-tauri/Cargo.toml
cargo test --lib --manifest-path src-tauri/Cargo.toml   # macOS 不需要 --release
```

> **为什么 Windows 需要 `--release`？** debug 模式的 `cargo test` 会静态链接完整 Tauri 栈，产生 >200 MiB 的测试二进制，会在测试代码运行之前就触发 Windows 加载器 bug（`STATUS_ENTRYPOINT_NOT_FOUND`）。release 模式的优化能把体积压到阈值以下。这是 Windows 专属问题，macOS 不受影响。
...
|---------------|-----------------------------------------------------------------|
| Windows exe   | `src-tauri/target/release/ipet.exe`                             |
| NSIS 安装包   | `src-tauri/target/release/bundle/nsis/iPet_0.3.1_x64-setup.exe` |
| Portable zip  | `src-tauri/target/release/bundle/zip/iPet_0.3.1_x64_portable.zip` |
| macOS DMG     | `src-tauri/target/release/bundle/dmg/iPet_0.3.1_aarch64.dmg`   |

> **macOS 用户**：DMG 未做公证。首次启动请**右键点击 app → 打开**，在弹出的对话框中点**打开**；也可以通过命令行去隔离标记：
> ```zsh
> xattr -d com.apple.quarantine /Applications/iPet.app
> ```
> 只需操作一次，之后即可正常双击运行。
...
- macOS / Linux 冒烟测试（Windows + macOS CI 已跑通；Linux 代码可移植但未验证）。
